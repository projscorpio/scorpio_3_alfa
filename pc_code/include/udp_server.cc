...
char* UDP_Server::rcv_data(char &response){
  char* buffer[MAXLINE];
  int len=0, n=0;
  n=recvfrom(this.sock_fd, (char*) buffer, MAXLINE, MSG_WAITALL,
      (struct sockaddr*) &this.client_addr, &len);
  buffer[n]='\n';
  sendto(this.sock_fdm (const char*) response, strlen(response),
...
char* UDP_Client::send_data(char* msg){
  ...
  n=recvfrom(this.sock_fd, (char*) buffer, MAXLINE, 
      MSG_WAITALL, (struct sockaddr*) &this.serv_addr, &len);
  buffer[n]='\n';
  return buffer;
}
